...
const REQUIRED_COLUMNS = ["lessonnumber", "lessonname", "itemtype", "itemtitle", "order"] as const;
const VALID_ITEM_TYPES = ["concept", "exercise", "songsheet"] as const;

type RequiredColumn = typeof REQUIRED_COLUMNS[number];

// ─── Native XLSX parser ───────────────────────────────────────────────────────
// Parses .xlsx (Open XML) without external dependencies.
// .xlsx is a ZIP containing xl/worksheets/sheet1.xml + xl/sharedStrings.xml
...
async function readZipEntry(
  ...
  while (offset < bytes.length - 4) {
    ...
    // Advance — if data descriptor flag set (bit 3), skip 12 extra bytes after data
 ...
 * Returns { rows, error } — error is non-null if parsing fails.
 */
async function parseXlsxBuffer(
  buffer: ArrayBuffer,
): Promise<{ rows: Record<string, string>[]; error: string | null }> {
  try {
    // Read shared strings (for string-type cells)
    ...
    if (!sheetXml) {
      return { rows: [], error: "Could not read sheet1 from the Excel file." };
    }
    ...
    const headers: string[] = cellsInHeader.map(cell => {
      ...
      if (tAttr?.[1] === "inlineStr") {
        const t = parseXmlText(cell, "t");
        return stripXmlTags(t[0] ?? "");
      }
    ...
    for (let ri = 1; ri < rowMatches.length; ri++) {
      ...
      for (let ci = 0; ci < cells.length; ci++) {
        ...
        else if (tAttr?.[1] === "inlineStr") {
          const t = parseXmlText(cell, "t");
          value   = stripXmlTags(t[0] ?? "");
        } else {
          value = stripXmlTags(raw);
        }
...
/** Extract 0-based column index from cell reference (e.g. "B3" → 1). */
...
function validateImport(
  ...
  for (const col of REQUIRED_COLUMNS) {
    if (!normalised.includes(col)) {
      columnErrors.push(`Missing required column: "${col}"`);
    }
  }
  ...
  rows.forEach((r, i) => {
    ...
    if (!r.lessonName?.trim()) {
      rowErrors.push(`Row ${rowNum}: lessonName is required`);
    }
    if (!r.itemTitle?.trim()) {
      rowErrors.push(`Row ${rowNum}: itemTitle is required`);
    }
    ...
    if (isNaN(r.order) || r.order < 1) {
      rowErrors.push(`Row ${rowNum}: order must be a positive number`);
    }

    // Duplicate (lessonNumber + order) within this file
    const key = `${r.lessonNumber}:${r.order}`;
    if (seenKeys.has(key)) {
      rowErrors.push(`Row ${rowNum}: duplicate (lessonNumber ${r.lessonNumber}, order ${r.order})`);
    } else {
      seenKeys.add(key);
    }
  });
  ...
  return {
    columnErrors,
    rowErrors,
    valid: columnErrors.length === 0 && rowErrors.length === 0,
  };
}
...
function LessonImportContent() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (error) {
        setValidation({ columnErrors: [error], rowErrors: [], valid: false });
        return;
      }

      if (rawRows.length === 0) {
        setValidation({ columnErrors: ["File has no data rows."], rowErrors: [], valid: false });
        return;
      }
  ...
  async function handleImport() {
    if (!user || !scopeId.trim()) {
      toast("Enter a center ID or student ID before importing.", "error");
      return;
    }
    if (!validation?.valid) {
      toast("Fix all validation errors before importing.", "error");
      return;
    }
    if (preview.length === 0) {
      toast("No rows to import.", "error");
      return;
    }
    ...
    try {
      ...
      // Normalise itemType to lowercase before sending to service
      const normalised = preview.map(r => ({
        ...r,
      ...
      const res = await bulkImportLessons(
        normalised,
        scope,
        user.uid,
        role ?? "admin",
  ...
  return (
    <div>
      ...
      <div style={s.infoCard}>
        <div style={s.infoTitle}>Required columns (exact names, case-insensitive):</div>
        <div style={s.chipRow}>
          {(["lessonNumber", "lessonName", "itemType", "itemTitle", "order"] as const).map(col => (
            <span key={col} style={s.chip}>{col}</span>
          ))}
        </div>
        <div style={s.infoNote}>
          itemType must be one of: <strong>concept</strong>, <strong>exercise</strong>, <strong>songsheet</strong>.
          One row per item. Multiple rows sharing the same lessonNumber are grouped into one lesson.
        </div>
      </div>

      {/* Scope selector */}
      <div style={s.card}>
        <div style={s.cardTitle}>Assign to</div>
        <div style={s.row}>
          <label style={s.radioLabel}>
            <input type="radio" checked={scopeType === "center"}
              onChange={() => { setScopeType("center"); setScopeId(""); }} style={{ marginRight: 6 }} />
            Center
          </label>
          <label style={s.radioLabel}>
            <input type="radio" checked={scopeType === "student"}
              onChange={() => { setScopeType("student"); setScopeId(""); }} style={{ marginRight: 6 }} />
            Student
          </label>
        </div>
        <input
          ...
          placeholder={scopeType === "center" ? "Enter Center ID…" : "Enter Student UID…"}
          style={s.input}
        />
      </div>
      ...
      {validation?.valid && preview.length > 0 && (
        <div style={s.successBox}>
          ✓ {preview.length} row{preview.length !== 1 ? "s" : ""} validated successfully
        </div>
      )}
        ...
        <div style={s.section}>
          ...
          <div style={s.tableWrapper}>
            <table style={s.table}>
              <thead>
                <tr>
                  {["#", "Lesson No.", "Lesson Name", "Item Type", "Item Title", "Order"].map(h => (
              ...
              <tbody>
                {preview.slice(0, 20).map((row, i) => (
                  <tr key={i} style={i % 2 === 0 ? s.rowEven : s.rowOdd}>
                    ...
                    <td style={{ ...s.td, ...s.mono }}>{row.order}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const s: Record<string, React.CSSProperties> = {
  ...
  row:          { display: "flex", gap: 20, marginBottom: 12 },
  radioLabel:   { display: "flex", alignItems: "center", fontSize: 13, fontWeight: 500, color: "#111827", cursor: "pointer" },
  input:        { width: "100%", maxWidth: 340, padding: "8px 10px", border: "1px solid #d1d5db", borderRadius: 6, fontSize: 13, outline: "none", background: "#fff", color: "#111827" },
