...
export function AdminsContent() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // 4. Write Firestore user doc (doc ID = Firebase Auth UID). plainPassword is
      //    kept so the Founder-only Users page can display working credentials.
...
function DeleteConfirm({ name, uid, role, currentUserUid, onDeleted, onError, onClose }: {
  ...
  async function doDelete() {
    ...
    setBusy(true);
    try {
      const res = await deleteUserRecord(uid, role, currentUserUid, "founder");
